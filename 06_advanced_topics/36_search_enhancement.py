...
涵盖主题：
  1. 多源数据建模（UnifiedDocument）
  2. BM25 中文关键词搜索（拼写容错）
  3. ChromaDB 向量语义搜索（近义词理解）
  4. RRF 融合排序与降级策略
  5. 标签关联推荐
  6. 编排引擎

运行：python 06_advanced_topics/36_search_enhancement.py
"""
...
def tokenize(text: str) -> list[str]:
    ...
    同时保留词级（语义）和字级（部分匹配）的 token，
    使得查询「退kuan」中「退」字能匹配到「退款」相关文档。
    """
